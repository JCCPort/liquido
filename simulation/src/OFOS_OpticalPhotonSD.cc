...
G4bool OFOS_OpticalPhotonSD::process_hit(const G4Step *aStep, const G4TouchableHistory *) {
    ...
    G4int gen_proc_type = (theTrack->GetCreatorProcess() ? theTrack->GetCreatorProcess()->GetProcessType()
                                                         : my_undefined_proc);
    G4int gen_proc_subtype = (theTrack->GetCreatorProcess() ? theTrack->GetCreatorProcess()->GetProcessSubType()
                                                            : my_undefined_proc);
    ...
    if (gen_proc_type == G4ProcessType::fElectromagnetic) {
        switch (gen_proc_subtype) {
            ...
            case G4EmProcessSubType::fScintillation :
                my_proc_id = my_scintillation;
                break;
            case G4EmProcessSubType::fCerenkov      :
                my_proc_id = my_cherenkov;
                break;
            default:
                G4cout << "Unknown optical photon creation process" << G4endl;
                break;
        }
    }
