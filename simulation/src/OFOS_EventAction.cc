...
OFOS_EventAction::EndOfEventAction(const G4Event *event) {
    ...
    if (vert_fiber_hit_collection_id_ == -1)
        vert_fiber_hit_collection_id_ = G4SDManager::GetSDMpointer()->GetCollectionID("VertFiberHitsCollection");
    if (hori_fiber_hit_collection_id_ == -1)
        hori_fiber_hit_collection_id_ = G4SDManager::GetSDMpointer()->GetCollectionID("HoriFiberHitsCollection");
    if (vessel_hit_collection_id_ == -1)
        vessel_hit_collection_id_ = G4SDManager::GetSDMpointer()->GetCollectionID("VesselHitsCollection");
    if (vert_sipmt_hit_collection_id_ == -1)
        vert_sipmt_hit_collection_id_ = G4SDManager::GetSDMpointer()->GetCollectionID("VertSiPmtHitsCollection");
    if (hori_sipmt_hit_collection_id_ == -1)
        hori_sipmt_hit_collection_id_ = G4SDManager::GetSDMpointer()->GetCollectionID("HoriSiPmtHitsCollection");
